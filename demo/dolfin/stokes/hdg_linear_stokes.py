...
for run_no, n_ele in enumerate(n_eles):
    ...
    n = FacetNormal(mesh)
    gN = (grad(u_soln) - p_soln*Identity(2))*n
    ...
    def F_v(u, grad_u, p_local=None):
        if p_local is None:
            p_local = pbar
        return grad_u - p_local*Identity(2)

    F = inner(F_v(u, grad(u), p), grad(v)) * dx - dot(gN, v)*dsN
    ...
    print("u e", l2error_u, h1error_u, "p e", l2error_p, h1error_p)
